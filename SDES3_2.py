...
def binarymain():
    def encrypt_text():
        ...
        ciphertext = encrypt_binary_text(plaintext, key)
        ciphertext_entry.delete(0, tk.END)
        ciphertext_entry.insert(0, ciphertext)
        messagebox.showinfo("Encryption", "成功使用给定密钥加密明文")
    ...
    def decrypt_text():
        ...
        plaintext = decrypt_binary_text(ciphertext, key)
        plaintext_entry.delete(0, tk.END)
        plaintext_entry.insert(0, plaintext)
        messagebox.showinfo("Decryption", "成功使用给定密钥解密密文")
    ...
    encrypt_button = tk.Button(root, text="随机生成密钥", command=random_key)
    encrypt_button.pack()
    input_label = tk.Label(root, text="输入明文或密文（二进制）:")
    input_label.pack()
    input_text = tk.Entry(root)
    input_text.pack()
    ...
    decrypt_button = tk.Button(root, text="测试", command=test_encrypt)
    decrypt_button.pack()
...
def select():
    ...
    button = tk.Button(root, text="二进制数加解密GUI", command=activate_binarymain)
    button.pack(pady=20)
    button = tk.Button(root, text="ASCII码加解密GUI", command=activate_asc)
    button.pack(pady=20)
    button = tk.Button(root, text="中文加解密GUI", command=activate_main)
    button.pack(pady=20)
